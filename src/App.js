...
import { MY_MENU, CATEGORIES } from "./menu-data";
...
// ── TODAY ─────────────────────────────────────────────────────────────────────
function Today({ totals, goals, viewLog, steps, water, tdee, isToday, viewDate, onSteps, onWater, onRemove, onEdit, onGoToLog }) {
...
function ChatLog({ ingredients, recipes, viewLog, viewDate, goals, onAdd, onRemoveByName, onSaveIng }) {
  const today = dateStr(0);
  ...
  async function send() {
    ...
    try {
      ...
      // Parse ```json blocks (more reliable with Gemini)
      const jsonMatch = raw.match(/```json\s*([\s\S]*?)```/);
      const jsonMatch2 = raw.match(/```\s*([\s\S]*?)```/); // fallback without language tag
      ...
      if (match) {
        try {
          ...
          if (parsed.message && parsed.message.trim()) displayText = parsed.message;
        } catch(e) { console.log("JSON parse error:", e, match[1]); }
      }
      if (!displayText || displayText.length < 2) displayText = parsed?.message || "noted.";
...
// ── MY MENU ───────────────────────────────────────────────────────────────────
function MenuTab({ pantry, onAddToLog }) {
        ...
        <div key={dish.id} onClick={()=>setSelected(dish)} style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:14, padding:14, cursor:"pointer" }}
          ...
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start" }}>
            <div>
              ...
              <div style={{ fontSize:11, color:C.sub, marginTop:3 }}>{dish.cal[0]}–{dish.cal[1]} kcal · P:{dish.protein[0]}–{dish.protein[1]}g · {dish.cookTime}</div>
            </div>
...
function Planner({ goals, log, onLogMeal }) {
  ...
  async function autoGenerate() {
    setGenerating(true);
    try {
      const prompt = `Generate a 7-day meal plan using ONLY these dishes: ${MY_MENU.map(d=>d.name).join(", ")}.
Daily calorie goal: ${goals.cal} kcal, protein: ${goals.protein}g.
Return ONLY raw JSON: { "mon":{"breakfast":"dish name","lunch":"dish name","dinner":"dish name"}, "tue":{...}, ... }
Keys: mon,tue,wed,thu,fri,sat,sun. Values must be exact dish names from the list. Pick realistic combinations.`;
      const raw = await callAI([{role:"user",content:prompt}], "You are a meal planning assistant. Return only JSON.");
...
function Library({ ingredients, recipes, onSaveIng, onSaveRec }) {
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
      ...
      {view==="recipes"&&(
        <>
          {recipes.length===0&&<div style={{ textAlign:"center", color:C.muted, fontSize:13, padding:28 }}>no recipes yet.</div>}
          {recipes.map(rec=>(
            <div key={rec.id} style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:12, padding:14 }}>
              <div style={{ display:"flex", justifyContent:"space-between" }}>
                <div>
                  ...
                  <div style={{ fontSize:11, color:C.sub }}>{Math.round(rec.cal||0)} kcal · P:{Math.round(rec.protein||0)}g</div>
                </div>
