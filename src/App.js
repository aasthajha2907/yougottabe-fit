...
export default function App() {
  ...
  return (
    <div style={{ background: C.bg, minHeight: "100vh", fontFamily: "'Plus Jakarta Sans','Segoe UI',sans-serif", color: C.text, maxWidth: "clamp(380px, 44vw, 800px)", margin: "0 auto", display: "flex", flexDirection: "column" }}>
      <style>{`
        @import url('https://fonts.googleapis.com/css2?family=Plus+Jakarta+Sans:wght@400;600;700;800&display=swap');
        *{box-sizing:border-box;margin:0;padding:0;}
        html{font-size:clamp(13px, 1.1vw, 20px);}
...
function Supplements() {
  ...
  function saveSupps(v) { setSupps(v); save("ft_supps", v); }
  function toggle(id) {
    const next = { ...todayChecked, [id]: !todayChecked[id] };
    const nextLog = { ...checked, [today]: next };
    setChecked(nextLog); save("ft_supp_log", nextLog);
  }
  function addSupp() {
    if (!newName.trim()) return;
    saveSupps([...supps, { id: Date.now(), name: newName.trim() }]);
    setNewName(""); setAdding(false);
  }
  function deleteSupp(id) { saveSupps(supps.filter(s => s.id !== id)); }
    ...
    <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 14, padding: 16 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
        <div>
          ...
          {supps.length > 0 && (
            <div style={{ fontSize: 11, color: doneCount === supps.length ? C.green : C.sub, marginTop: 2 }}>
              {doneCount}/{supps.length} taken today {doneCount === supps.length ? "✓ all done!" : ""}
            </div>
          )}
        </div>
        <button onClick={() => setAdding(p => !p)} style={{ background: C.accentDim, border: `1px solid ${C.accent}44`, borderRadius: 7, color: C.accent, fontSize: 12, fontWeight: 700, cursor: "pointer", padding: "5px 12px" }}>
          {adding ? "cancel" : "+ Add"}
        </button>
      </div>
      ...
      {adding && (
        <div style={{ display: "flex", gap: 8, marginBottom: 12 }}>
          <input value={newName} onChange={e => setNewName(e.target.value)}
            ...
            placeholder="e.g. Vitamin D3, Omega 3, B12…"
      ...
      {supps.length === 0 && !adding && (
        <div style={{ color: C.muted, fontSize: 12, textAlign: "center", padding: "12px 0" }}>No supplements added yet. Hit + Add to get started.</div>
      )}
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
        {supps.map(s => {
          ...
          return (
            <div key={s.id} style={{ display: "flex", alignItems: "center", gap: 12, padding: "10px 12px", background: done ? C.green + "12" : C.surface, border: `1px solid ${done ? C.green + "44" : C.border}`, borderRadius: 10, cursor: "pointer", transition: "all 0.2s" }}
              onClick={() => toggle(s.id)}>
              ...
              <span style={{ fontSize: 13, fontWeight: 600, color: done ? C.green : C.text, flex: 1, textDecoration: done ? "none" : "none" }}>{s.name}</span>
              <button onClick={e => { e.stopPropagation(); deleteSupp(s.id); }}
                style={{ background: "none", border: "none", color: C.muted, cursor: "pointer", fontSize: 16, lineHeight: 1, padding: 2 }}>×</button>
            </div>
          );
        })}
      </div>
...
function Today({ totals, goals, todayLog, steps, water, bmr, tdee, baseline, onSteps, onWater, onRemove }) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
      ...
      {/* macros */}
      <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 14, padding: 16 }}>
        <div style={{ fontSize: 11, fontWeight: 700, color: C.sub, textTransform: "uppercase", letterSpacing: 1, marginBottom: 12 }}>Macronutrients</div>
        <div style={{ display: "flex", gap: 8, marginBottom: 12 }}>
...
function FatLoss({ log, goals, tdee, baseline, profile }) {
  ...
  // streak: consecutive deficit days
  let streak = 0;
